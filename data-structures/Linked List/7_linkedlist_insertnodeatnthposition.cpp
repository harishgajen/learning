...
{
    Node* temp1 = new Node(); // temp1 is a pointer to the Node created in the heap
    ...
    // An easy way to remember this is that, temp1->next starts as null, so we'd have to change this first.
    // Then, temp2->next can't be temp1->next, because that would be what it already is, so it must simply point to temp1
    // This is a temporary solution to remembering, but if I spend some time to think about it, it is pretty easy. Drawing out the nodes in paint may even help, and seeing where 
    // the pointers should change.
